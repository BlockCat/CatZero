...
pub struct AlphaZero<'a, A, B> where A: GameAction + Sync, B: AlphaZeroState<A> + Sync {    
    player_1: AlphaAgent<'a, A, B>,
    player_2: AlphaAgent<'a, A, B>,    
}

impl<'a,A, B> AlphaZero<'a, A, B> where A: GameAction + Sync, B: AlphaZeroState<A> + Sync {
    ...
    fn play_game(&mut self, player_first: Player, player_second: Player) -> (Option<Player>, Vec<(B, Tensor<f32>)>) {
        ...
        while !current_state.is_terminal() {
            ...
            let (player_action, probs) = if let Some((player_action, probs)) = player_first.get_alpha_action(current_state.clone()) {
                (player_action, probs)
            } else {
                break;
            };
            ...
            let (player_action, probs) = if let Some((player_action, probs)) = player_second.get_alpha_action(current_state.clone()) {
                (player_action, probs)
            } else {
                break;
            };
